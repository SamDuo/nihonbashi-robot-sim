...
A 36-second demonstration video accompanies this document as `outputs/reports/demo_video.mp4`. It shows the twin on the corrected demand layer: the approach to the block, an orbit, the four metric layers, the S1 and S2 demand scenarios and the supply options with their emissions deltas, and the agent layer under a 07:00 to 19:00 clock sweep.
